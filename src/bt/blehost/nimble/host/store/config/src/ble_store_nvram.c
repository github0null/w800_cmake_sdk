...
#define CCCD_VALID_BITS_MASK      (0xFFFFFFFC)
    ...
    for(i = 0; i< 5; i++)
    {
        nv_tag_valid = btif_config_get_sec_cccd_item(i, &addr, &our_sec,sizeof(our_sec), &peer_sec,sizeof(peer_sec), &cccd_info[0], sizeof(cccd_info));
        if(nv_tag_valid)
        {
...
int ble_store_config_persist_cccds()
{
    ...
    for(i = 0; i< prefer_index; i++)
    {
        ...
        btif_config_store_cccd(nv_idx,&cccd_nvram_array[i].addr, cccd_nvram_array[i].offset, ptr_byte, sizeof(ble_store_value_t)*MYNEWT_VAL(BLE_STORE_MAX_CCCDS));
    }
...
int ble_store_config_persist_peer_secs()
{
    ...
    for(i = 0; i<ble_store_config_num_peer_secs; i++)
    {
        ...
        if(nv_idx < 0)
        {
           BLE_HS_LOG(ERROR,"PEER SEC Full, impossible\r\n");
           return -1;
        }
        //printf("ble_store_config_persist_peer_secs,found =%d, nv_idx=%d\r\n", found, nv_idx);
        btif_config_store_peer_sec(nv_idx,&ble_store_config_peer_secs[i].peer_addr, (void*)&ble_store_config_peer_secs[i], sizeof(ble_store_config_peer_secs[i]));
    }
...
int ble_store_config_persist_our_secs()
{
    ...
    for(i = 0; i<ble_store_config_num_our_secs; i++)
    {
        ...
        if(nv_idx < 0)
        {
           BLE_HS_LOG(ERROR,"OUR SEC Full, impossible\r\n");
           return -1;
        }
        //printf("ble_store_config_persist_our_secs, found =%d, nv_idx=%d\r\n", found, nv_idx);
        btif_config_store_our_sec(nv_idx,&ble_store_config_our_secs[i].peer_addr, (void*)&ble_store_config_our_secs[i], sizeof(ble_store_config_our_secs[i]));
    }
...
{
    if(ble_npl_callout_is_active(&g_defer_store_timer)) ble_npl_callout_stop(&g_defer_store_timer);
    
    ble_npl_callout_reset(&g_defer_store_timer,ble_npl_time_ms_to_ticks32(3000));
}
static void ble_defer_store_config_flush_cb(struct ble_npl_event *evt)
{
    btif_config_flush(1);
}
...
{
    ble_npl_callout_init(&g_defer_store_timer, nimble_port_get_dflt_eventq(), ble_defer_store_config_flush_cb, NULL);
...
{
    ble_npl_callout_deinit(&g_defer_store_timer);
}
