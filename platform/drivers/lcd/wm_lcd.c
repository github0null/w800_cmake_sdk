...
void tls_lcd_fresh_ratio(uint8_t com_num, uint16_t freq)
{
	...
	WM_LCD->FRAMECNT = RTC_CLK/freq/com_num;
}
...
{
	WM_LCD->CTRL = 0;
	WM_LCD->CTRL = opts->bias | opts->duty | opts->vlcd | (1 << 12);
